...
pub trait DecimalInterface: Scalar {
    ...
    fn from_raw(num: Self::LiteralValue, scale: u32) -> Self;
...
impl Decimal32 {
    pub fn from_raw(num: i32, scale: u32) -> Self {
        Self(Some(Decimal::new(num as i64, scale)))
    }
    ...
    pub fn mantissa(&self) -> Option<i32> {
        if let Some(d) = self.as_ref() {
            ...
            if mantissa > i32::MAX as i128 || mantissa < i32::MIN as i128 {
                None
            } else {
    ...
    pub fn rescale(&mut self, scale: u32) {
        self.as_mut().map(|d| d.rescale(scale));
    }
}

impl Decimal64 {
    pub fn from_raw(num: i64, scale: u32) -> Self {
        Self(Some(Decimal::new(num, scale)))
    }
...
impl Decimal128 {
    pub fn from_raw(num: i128, scale: u32) -> Self {
        Self(Some(Decimal::from_i128_with_scale(num, scale)))
    }
